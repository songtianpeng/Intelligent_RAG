...
def generate_manual():
    ...
    pdf.multi_cell(0, 6.5, "HTF1200型卧式注塑机是本公司主力生产设备，采用伺服电机驱动和"
                            "双缸注射系统，适用于热塑性塑料的精密成型加工。")
    pdf.ln(4)
    pdf.set_font("st", "B", 12)
    pdf.cell(0, 8, "1.2 主要技术参数"); pdf.ln(8)
    pdf.set_font("st", size=11)
    for k in ["螺杆直径：80/90/100 mm", "最大锁模力：1200 吨", "注射压力：180 MPa"]:
        pdf.cell(0, 6.5, "  * " + k); pdf.ln(6.5)
    pdf.ln(4)
    pdf.set_font("st", "B", 14)
...
def generate_fault_codes():
    ...
    for title, codes in [("第一章 电气系统故障（E系列）", E_CODES),
        ...
        for code, name, cause, solution in codes:
            ...
            pdf.set_font("st", size=11)
            pdf.cell(0, 6.5, "  可能原因：" + cause); pdf.ln(6.5)
            pdf.cell(0, 6.5, "  解决方案：" + solution); pdf.ln(8)
...
def generate_maintenance():
    ...
    for section_title, items in [("第一章 日常维保（每班次）", DAILY),
        ...
        for item, method, std, action in items:
            pdf.set_font("st", "B", 11)
            pdf.cell(0, 7, f"  {item}"); pdf.ln(7)
            pdf.set_font("st", size=10)
            pdf.cell(0, 6, f"    方法：{method}  |  合格标准：{std}  |  异常处理：{action}")
            pdf.ln(6)
        pdf.ln(4)
